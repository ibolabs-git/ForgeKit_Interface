...
| ID | Problem | Fajl | Status |
|----|---------|------|--------|
| OPT-01 | `react-syntax-highlighter` full import (+2MB bundle) | `MessageBubble.tsx` | ✅ v0.9.5 |
| OPT-03 | `SidePanel` pretplaćen na cijeli `messages` array | `SidePanel.tsx` | ✅ v0.9.5 |
| OPT-05 | `matchIds.includes()` O(n) u render loopu | `ChatWindow.tsx` | ✅ v0.9.5 |
| OPT-06 | Memory leak — stream listeneri se ne čiste pri unmount | `InputBar.tsx` | ✅ v0.9.5 |
| OPT-07 | `reprimePreviewText` useMemo računa se i kad je preview zatvoren | `SidePanel.tsx` | ✅ v0.9.5 |
| OPT-08 | `LeftPanel` pretplaćen na `messages` array samo za `.length` | `LeftPanel.tsx` | ✅ v0.9.5 |
...
| ID | Problem | Fajl | Status |
|----|---------|------|--------|
| SEC-04 | Hardkodovani `encryptionKey` u electron-store | `store.ts` | ✅ v1.0.0 |
| SEC-05 | `systemPrompt` putuje kroz IPC od renderer-a | `InputBar.tsx` + providers | ✅ v1.0.0 |
| OPT-02 | `appendStreamToken` O(n) na svaki token — streaming performance | `forgekit.store.ts` | ✅ v1.0.0 |
...
*Posljednje ažuriranje: v1.0.0 — 2026-05-19 (Prolaz 4 završen; svi audit itemi zatvoreni)*
